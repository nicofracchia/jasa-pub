...
class UsuariosController extends AbstractController
{
    ...
    public function index(UsuariosRepository $usuariosRepository): Response
    {
        return $this->render('usuarios/index.html.twig', [
            'usuarios' => $usuariosRepository->findAllVigentes(),
        ]);
    }
    ...
    public function new(Request $request, UserPasswordEncoderInterface $encoder): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $clave = $encoder->encodePassword($usuario, $request->request->get('usuarios')['password']);
            $usuario->setPassword($clave);
            $roles = array('ROLE_ADMIN', 'ROLE_PRODUCTOS', 'ROLE_REPARACIONES', 'ROLE_ALMACENES', 'ROLE_PROVEEDORES', 'ROLE_CLIENTES', 'ROLE_COTIZACIONES', 'ROLE_VENTAS', 'ROLE_COMPRAS');
     ...
     */
    public function edit(Request $request, Usuarios $usuario, UserPasswordEncoderInterface $encoder): Response
    {
        $form = $this->createForm(UsuariosType::class, $usuario);
        $form->handleRequest($request);
    ...
    public function delete(Request $request, Usuarios $usuario): Response
    {
        if ($this->isCsrfTokenValid('delete'.$usuario->getId(), $request->request->get('_token'))) {
            $entityManager = $this->getDoctrine()->getManager();
            $usu = $entityManager->getRepository(Usuarios::class)->find($usuario->getId());
            $usu->setEliminado(1);
            $entityManager->flush();
            /*
            $entityManager = $this->getDoctrine()->getManager();
